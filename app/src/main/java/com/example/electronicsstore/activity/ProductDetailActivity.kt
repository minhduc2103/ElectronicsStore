...
class ProductDetailActivity : BaseActivity() {
    ...
    private fun initData() {
        if (mProduct == null) return
        loadUrlBanner(mProduct!!.banner, imgProduct!!)
        ...
        if (mProduct!!.info != null) {
            val temp =
                mProduct!!.info!!.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val strInfo = StringBuilder()
            for (s in temp) {
                if (strInfo.isEmpty()) {
                    strInfo.append(s.trim { it <= ' ' })
                } else {
                    strInfo.append("\n").append(s.trim { it <= ' ' })
                }
            }
            tvInfo!!.text = strInfo.toString()
        }
